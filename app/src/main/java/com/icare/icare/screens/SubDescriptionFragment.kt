...
class SubDescriptionFragment : BaseFragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val subDescriptionAdapter = SubDescriptionAdapter(
            ...
            if (subscriptionPlanId > 0) {
                ...
                val action = SubDescriptionFragmentDirections.actionMySubToPayment(position)
        ...
        call.enqueue(object : Callback<Void> {
            override fun onResponse(call: Call<Void>, response: Response<Void>) {
                if (response.isSuccessful) {
                   findNavController().navigate(SubDescriptionFragmentDirections.actionSubToPaymentA())
                } else {
